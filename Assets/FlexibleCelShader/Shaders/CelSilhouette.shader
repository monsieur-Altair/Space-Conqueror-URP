	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f input) : SV_Target
			{
				...
				// Sample textures
				fixed4 col = tex2D(_MainTex, input.uv * _MainTex_ST.xy + _MainTex_ST.zw);
				fixed3 tangentNormal = tex2D(_NormalTex, input.uv * _NormalTex_ST.xy + _NormalTex_ST.zw) * 2 - 1;
				fixed4 emmision = tex2D(_EmmisTex, input.uv * _EmmisTex_ST.xy + _EmmisTex_ST.zw);

				// Get normal
				float3 worldNormal = float3(input.worldTangent * tangentNormal.r + input.worldBitangent * tangentNormal.g + input.worldNormal * tangentNormal.b);
